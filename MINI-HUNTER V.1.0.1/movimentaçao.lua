...
function monster_move(dt, px, py, monster_vetor,window_w,window_h)
    for i=#monster_vetor, 1, -1 do
        local monster = monster_vetor[i]
        if monster.tipo < 2 and monster.movedelay < 0 then
            if px > monster.origemx then
                monster.x = monster.x + monster.sp * dt
            end
            if px < monster.origemx then
                monster.x = monster.x - monster.sp * dt
            end
            if py > monster.origemy then
        ...
        elseif monster.tipo >= 2 then
            if monster.contatiro >= 3 then
                monster.x = love.math.random(px - 400 ,px + 400 )
                monster.y = love.math.random(py - 400 ,py + 400 )
                monster.contatiro = 0
            ...
            elseif  (((px-monster.x)^2 + (py - monster.y)^2)^(1/2)) < 175 then
                if px > monster.origemx then
                    monster.x = monster.x - monster.sp/0.75 * dt
                end
                if px < monster.origemx then
                    monster.x = monster.x + monster.sp/0.75 * dt
                end
                if py > monster.origemy then
                    monster.y = monster.y - monster.sp/0.75 * dt
                end
                if py < monster.origemy then
                    monster.y = monster.y + monster.sp/0.75 * dt
                end
